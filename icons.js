/**
 * FMHY Supercharged — SVG Icon System
 * =====================================================================
 * Replaces all emojis with clean, consistent SVG icons that match
 * VitePress's icon style (24x24, stroke-based, currentColor).
 *
 * Usage:
 *   FMHY.Icon.render("bookmark")     // returns SVG element
 *   FMHY.Icon.inject(button, "star") // injects icon into element
 *   FMHY.Icon.html("search")         // returns SVG HTML string
 *
 * All icons use `currentColor` so they inherit text color.
 * Stroke width: 1.5 (matches VitePress `.vp-icon` style).
 */
(function (global) {
  "use strict";
  global.FMHY = global.FMHY || {};

  /** @type {Record<string, string>} SVG path content for each icon */
  const ICONS = {
    // Navigation / general
    ...
    "bookmark-nav": '<polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/>',
    ...
    "file-json": '<path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/><text x="8" y="17" font-size="6" font-family="monospace" fill="currentColor" stroke="none">{}</text>',
    "file-html": '<path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/><text x="7" y="17" font-size="5" font-family="monospace" fill="currentColor" stroke="none">&lt;/&gt;</text>',
    "file-md": '<path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/><text x="8" y="17" font-size="6" font-family="monospace" fill="currentColor" stroke="none">M</text>',
    "file-csv": '<path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/><text x="6" y="17" font-size="5" font-family="monospace" fill="currentColor" stroke="none">CSV</text>',
    ...
    "zap-filled": '<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" fill="currentColor"/>',
  ...
  /**
   * Render an SVG icon as an HTMLElement.
   ...
   */
  function render(name, size = 16) {
    const path = ICONS[name];
    if (!path) {
      console.warn("[FMHY Icon] Unknown icon:", name);
      return render("circle", size);
    }
  ...
  /**
   * Return an SVG icon as an HTML string (useful for innerHTML).
  ...
  /**
   * Inject an icon into a target element (replaces children).
   * @param {HTMLElement} target - Element to inject into
   * @param {string} name - Icon name
   * @param {number} [size=16] - Pixel size
   */
  function inject(target, name, size = 16) {
    if (!target) return;
    target.innerHTML = "";
    target.appendChild(render(name, size));
  }
  ...
  global.FMHY.Icon = { render, html, inject, has, list };
